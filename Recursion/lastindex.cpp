...
int lastIndex(int arr[],int n,int target,int currentIndex)
{
    if(currentIndex==n)
      return -1;
    
    int index=lastIndex(arr,n,target,currentIndex+1);
    if(index==-1 && arr[currentIndex]==target)
    {
          return currentIndex;
    }
    else
